...
export function ConnectSection() {
	...
	const integrationsInView = useInView(integrationsRef, {
		margin: "-150px 0px -60% 0px",
		once: false,
	});
	const sourcesInView = useInView(sourcesRef, {
		margin: "-150px 0px -60% 0px",
		once: false,
	});

	const mobilePlanInView = useInView(mobilePlanRef, {
		margin: "-100px",
		once: false,
	});
	const mobileIntegrationsInView = useInView(mobileIntegrationsRef, {
		margin: "-150px 0px -60% 0px",
		once: false,
	});
	const mobileSourcesInView = useInView(mobileSourcesRef, {
		margin: "-150px 0px -60% 0px",
		once: false,
	});
	...
	const desktopStatus = planInView
		? status === "idle"
			? "connecting"
			: status
		: status;
